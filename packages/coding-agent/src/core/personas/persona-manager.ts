...
export class PersonaManager {
  ...
  statusText(): string {
    ...
    const tail =
      this._selectedId !== DEFAULT_ID ? ` · ${this._selectedId}` : "";
  ...
  register(pi: ExtensionAPI): void {
    ...
    pi.registerCommand("persona", {
      description:
        "Select a persona (dropdown), or switch directly: /persona [status|<id>]",
      async handler(args, ctx) {
        ...
        if (action === "status") {
          ...
          if (p) {
            ctx.ui.notify(
              `persona: ${p.name} · ${p.profession} · ${self._selectedId}`,
              "info"
            );
          } else {
        ...
        if (self.switchTo(action)) {
          ...
          ctx.ui.notify(
            `unknown persona "${action}". Available: ${ids}`,
            "warning"
          );
        }
      },
  ...
  private async _pickPersona(ctx: ExtensionContext): Promise<void> {
    ...
    const chosen = await ctx.ui.custom<Persona | null>(
