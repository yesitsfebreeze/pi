...
describe("nvim tool contract", () => {
	...
	it("does not advertise a backend parameter neither tool honours", () => {
		const { client } = clientCapturing("[]");
		for (const name of ["nvim_search", "nvim_find_files"]) {
			const props = (toolNamed(client, name).parameters as { properties: Record<string, unknown> }).properties;
			expect(Object.keys(props), `${name} must not advertise an inert 'backend'`).not.toContain("backend");
		}
	});
});
